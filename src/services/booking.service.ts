// Handles complete booking lifecycle including creation with conflict checks, availability validation, status updates, and role-based booking retrieval.
...
const hasOverlap = (checkIn: any, checkOut: any) => ({
  $or: [
    {
...
const expirePendingBookings = async (houseId: any) => {
...
const rejectOverlappingBookings = async (confirmedBooking: any) => {
  await Booking.updateMany(
    {
...
export const createBooking = async ({
  ...
  try {
    ...
    if ((house.owner as any)._id.toString() === guestId.toString()) {
    ...
    const populatedBooking = await Booking.findById(booking._id)
      ...
      .populate({ path: "guest", select: "firstName email" });
...
export const fetchGuestBookings = async (guestId: any) => {
  ...
  return bookings.map((booking) => {
    const data: any = booking.toObject();
...
export const fetchHostBookings = async (hostId: any) => {
...
export const updateBookingStatus = async (
  ...
  if ((booking.house as any).owner.toString() !== hostId.toString()) {
...
export const cancelBooking = async (bookingId: string, guestId: any) => {
